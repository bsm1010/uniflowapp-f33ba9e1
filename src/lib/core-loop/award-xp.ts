  ...
  .handler(async ({ data }) => {
    ...
    for (const a of achievements) {
      ...
      switch (a.condition_type) {
        case "products":
          earned = productCount >= a.condition_value;
          break;
        case "published":
          earned = publishedCount >= a.condition_value;
          break;
        case "orders":
          earned = orderCount >= a.condition_value;
          break;
        case "revenue":
          earned = totalRevenue >= a.condition_value;
          break;
        case "customized":
          earned = hasCustomized;
          break;
        case "referrals":
          earned = referralCount >= a.condition_value;
          break;
        case "streak":
          earned = newLevel >= a.condition_value;
          break;
      }
      if (earned) {
        await supabaseAdmin
    ...
    const { data: allUnlockables } = await client.from("unlockables").select("*");
    ...
    for (const u of allUnlockables || []) {
      ...
      switch (u.requirement_type) {
        case "level":
          shouldUnlock = finalLevel >= u.requirement_value;
          break;
        case "xp":
          shouldUnlock = finalXp >= u.requirement_value;
          break;
        case "achievement":
          shouldUnlock = earnedAchievementCount >= u.requirement_value;
          break;
      }
      if (shouldUnlock) {

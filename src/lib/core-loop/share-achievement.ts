...
export const shareAchievement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await Promise.all([
      client.from("user_achievements").update({ shared: true }).eq("id", data.achievementId),
      client.from("xp_events").insert({ user_id: user.id, event_type: "share_achievement", xp_amount: 15, metadata: { achievement_id: data.achievementId } }),
      client.from("user_gamification").upsert({ user_id: user.id, xp: (await client.from("user_gamification").select("xp").eq("user_id", user.id).maybeSingle()).data?.xp + 15 }, { onConflict: "user_id" }),
    ]);

...
const PACK_DEFS = [
  {
    ...
    credits: 50,
    price: 500,
    badge: null as null | "popular" | "bestValue",
    icon: Zap,
...
const PLAN_DEFS = [
  {
    ...
    credits: 100,
    price: 1500,
    perks: ["basic1", "basic2", "basic3"] as const,
    highlight: false,
    icon: Star,
...
function CreditsPage() {
  ...
  const submit = useCallback(async () => {
    ...
    if (!file.type.startsWith("image/"))
      return toast.error(t("dashboard.credits.errors.mustBeImage"));
  ...
  return (
    <div className="relative max-w-6xl mx-auto space-y-10 pb-12">
          ...
          <div className="flex items-center gap-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 px-5 py-4">
            ...
            <div>
              <div className="text-xs uppercase tracking-wider text-white/70">
                {t("dashboard.credits.yourCredits")}
              </div>
              <div className="text-3xl font-bold leading-tight">
                {plan === "business" ? "∞" : (credits ?? 0)}
              </div>
        ...
        <Card className="relative overflow-hidden border-border/60 hover:shadow-lg transition-shadow group">
          ...
          <CardContent className="relative p-5 flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">
                {t("dashboard.credits.inviteFriends")}
              </div>
              <Link
          ...
          {PACK_DEFS.map((p) => {
            ...
            return (
              ...
              >
                <div
                  aria-hidden
                  className={`absolute inset-0 bg-gradient-to-br ${p.gradient} ${active ? "opacity-10" : "opacity-0 group-hover:opacity-[0.06]"} transition-opacity`}
                />
                <div
                  aria-hidden
                  className={`absolute -top-12 -right-12 size-40 rounded-full bg-gradient-to-br ${p.gradient} opacity-20 blur-2xl`}
                />
                ...
                <div className="relative">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    {t("dashboard.credits.addedAfterReview")}
                  </div>
                  {active && (
                    ...
                    {p.id === "business" ? (
                      <>
                        ...
                        <span className="font-semibold">
                          {t("dashboard.credits.trulyUnlimited")}
                        </span>
                      </>
                    ) : (
                      <>
                        ...
                        <span className="font-semibold">{p.credits}</span>
                        <span className="text-muted-foreground">
                          {t("dashboard.credits.monthlyCredits")}
                        </span>
                      </>
                    )}
                  </div>
                  ...
                  <ul className="mt-4 space-y-2 text-sm">
                    {p.perks.map((perk) => (
                      <li key={perk} className="flex items-start gap-2">
                        <div
                          className={`mt-0.5 size-4 grid place-items-center rounded-full bg-gradient-to-br ${p.gradient} text-white shrink-0`}
                        >
      ...
      <Card className="relative overflow-hidden border-border/60 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.15)]">
        ...
        <div
          aria-hidden
          className="absolute -top-24 -right-24 size-64 rounded-full bg-violet-500/10 blur-3xl"
        />
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-semibold text-xl font-display">
                {t("dashboard.credits.proofTitle")}
              </h3>
              <p className="text-sm text-muted-foreground mt-1">
                {t("dashboard.credits.transferVia")}
              </p>
            </div>
          </div>

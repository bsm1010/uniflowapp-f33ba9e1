...
export function ColumnsFooter({ tokens: t, brand, tagline, links, socials, copyright, slug }: ColumnsFooterProps) {
  const groups = [
    { title: "Shop", links: links.slice(0, Math.ceil(links.length / 2)) },
    { title: "Help", links: links.slice(Math.ceil(links.length / 2)) },
  ];

  return (
    <footer style={{ backgroundColor: t.surfaceStrong, borderTop: `1px solid ${t.border}` }}>
      <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <div className="sm:col-span-2 lg:col-span-2">
            ...
            <p className="mt-3 text-sm max-w-sm leading-relaxed" style={{ color: t.muted }}>{tagline}</p>
          ...
          {groups.map((group) => (
            <div key={group.title}>
              <h4 className="text-sm font-semibold mb-3" style={{ color: t.fg }}>{group.title}</h4>
              <ul className="space-y-2.5">
                {group.links.map((link) => (
                  <li key={link.href}>
                    <a href={link.href} className="text-sm transition-colors duration-200 hover:opacity-80" style={{ color: t.muted }}>
                      {link.label}
                    </a>
                  </li>
                ))}
      ...
      <div style={{ borderTop: `1px solid ${t.border}` }}>
        <div className="mx-auto max-w-7xl px-5 py-5 sm:px-8 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <p className="text-xs" style={{ color: t.muted }}>Powered by Fennecly</p>
        </div>
      </div>
    </footer>
  );

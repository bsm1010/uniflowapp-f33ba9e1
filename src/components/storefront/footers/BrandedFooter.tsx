...
export function BrandedFooter({ tokens: t, brand, tagline, links, socials, copyright, slug }: BrandedFooterProps) {
  return (
    <footer className="relative overflow-hidden">
      <div
        className="absolute inset-0"
        style={{
          background: `linear-gradient(135deg, ${t.primary}22, ${t.secondary}44, ${t.bg})`,
        }}
      />
      <div className="relative mx-auto max-w-7xl px-5 pt-20 pb-12 sm:px-8">
        <div className="flex flex-col items-center text-center">
          ...
          <h3 className="text-2xl font-bold font-display" style={{ color: t.fg }}>{brand}</h3>
          <p className="mt-2 text-sm max-w-md" style={{ color: t.muted }}>{tagline}</p>

          <div className="mt-8 flex gap-2">
            {socials.instagram && (
              <a href={socials.instagram} target="_blank" rel="noopener noreferrer"
                className="flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-200 hover:scale-110"
                style={{ backgroundColor: t.surfaceStrong, color: t.muted }}>
          ...
          <nav className="mt-8 flex flex-wrap items-center justify-center gap-1">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="px-5 py-2 text-sm rounded-full transition-all duration-200 hover:bg-white/5"
                style={{ color: t.muted }}
              >
                {link.label}
              </a>
      ...
      <div style={{ borderTop: `1px solid ${t.border}` }}>
        <div className="mx-auto max-w-7xl px-5 py-5 text-center relative">

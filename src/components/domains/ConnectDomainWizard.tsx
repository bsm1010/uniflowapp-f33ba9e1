            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Bring your own domain to your storefront in minutes.
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {step === 3 && domainRow && <Step3 row={domainRow} onNext={() => setStep(4)} />}
              {step === 4 && domainRow && (
                <Step4
                  verifying={verifying}
                  attempts={attempts}
...
function Step1({
  ...
  classified: ReturnType<typeof classifyDomain>;
  onSubmit: () => void;
  loading: boolean;
}) {
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Use a domain you already own — root, www, or subdomain.
        </p>
      </div>
      <div className="relative">
        ...
        <Input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && classified) onSubmit();
          }}
        />
      </div>
      {value && (
          ...
          {classified ? (
            <div className="flex items-center justify-between">
              <span className="text-foreground">
                Detected: <span className="font-mono font-medium">{classified.domain}</span>
              </span>
            ...
            <span className="text-destructive">
              Not a valid domain. Example: store.com or shop.mybrand.com
            </span>
          )}
        </div>
      )}
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Copy each record into your DNS provider's dashboard.
          </p>
        </div>
  ...
  const cards = [
    {
      ...
      desc: "Sign in to your domain provider dashboard.",
    },
    {
      icon: Server,
      title: "Find DNS settings",
      desc: "Look for 'DNS', 'Zone Editor', or 'Manage DNS'.",
    },
    {
      icon: Wand2,
      title: "Add the records",
      desc: "Paste each record exactly as shown in the previous step.",
    },
    ...
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {cards.map((c, i) => (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <div className="h-8 w-8 rounded-lg bg-primary/10 text-primary grid place-items-center text-sm font-semibold">
                {i + 1}
              </div>
        ...
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 flex gap-2 text-sm">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-0.5">
              {error} Most providers take 1–10 minutes to propagate.
            </div>
          </div>
        </div>
      )}
      ...
      <Accordion type="single" collapsible>
        <AccordionItem value="trouble">
          <AccordionTrigger className="text-sm">
            <HelpCircle className="h-4 w-4 mr-2" /> Troubleshooting & FAQ
          </AccordionTrigger>
          <AccordionContent className="space-y-3 text-sm text-muted-foreground">
            <Faq
              q="How long does DNS take?"
              a="Usually 1–10 minutes, but can take up to 24 hours depending on your provider and TTL."
            />
            <Faq
              q="My provider isn't listed"
              a="Any provider works. Just look for the DNS / Zone Editor section in their dashboard."
            />
            <Faq
              q="Do I need to keep both A and TXT records?"
              a="Yes — the A/CNAME routes traffic, and the TXT proves you own the domain."
            />
            <Faq
              q="When does SSL turn on?"
              a="Automatically, within ~60 seconds after DNS verifies successfully."
            />
            <Faq
              q="Can I use Cloudflare proxy (orange cloud)?"
              a="Yes, but disable proxying initially so we can verify, then re-enable it."
            />
          </AccordionContent>
        </AccordionItem>
      </Accordion>
...
function Step5({ domain, onClose }: { domain: string; onClose: () => void }) {
  return (
    <div className="text-center py-4">
      ...
      <Button onClick={onClose} className="mt-6 h-11 px-8">
        Done
      </Button>
    </div>
  );
